...
from ..errors import CTypeError, SemanticError
from ..utils import on, when
from .semantic_utils import Context, ErrorType, SemanticException, VoidType
...
class COOL_TYPE_BUILDER(object):
    ...
    def build_basic_types(self):
        ...
        iot.define_method(
            "out_string", ["x"], [strt], iot
        )  # Is SELF_TYPE in the manual
    ...
    def visit(self, node: ClassDeclarationNode):  # noqa:F811
        try:
            ...
            parent_type = (
                self.context.get_type(node.parent)  # type:ignore
    ...
    def visit(self, node: AttrDeclarationNode):  # noqa:F811
        atype = None
        try:
    ...
    def visit(self, node: FuncDeclarationNode):  # noqa:F811
        ret_type = ErrorType()
        params_type = [ErrorType()] * len(node.params)
        params_id = []
        try:
            ret_type = (
                self.context.get_type(node.type) if node.type != "void" else VoidType()
            )
        except SemanticException as e:
            self.errors.append(CTypeError(node.line, node.column, e.text))
