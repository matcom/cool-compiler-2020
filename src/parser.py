...
def p_class_definition(p):
    ...
    #if(len(p) == 7):
    #    p[0] = ClassNode(p[2], p[4], None, p.lineno)
    #else:
    #    p[0] = ClassNode(p[2], p[6], p[4], p.lineno)
...
def p_attribute_feature(p):
    ...
    #if(len(p) == 5):
    #    p[0] = AttributeFeatureNode(p[1], p[3], None, p.lineno)
    #else:
    #    p[0] = AttributeFeatureNode(p[1], p[3], p[5], p.lineno)
...
def p_function_feature(p):
    ...
    #if(len(p) == 10):
    #    p[0] = FunctionFeatureNode(p[1], [], p[5], p[7], p.lineno)
    #else:
    #    p[0] = FunctionFeatureNode(p[1], p[3], p[6], p[8], p.lineno)
...
def p_parameter(p):
    ...
    #p[0] = ParameterNode(p[1], p[3], p.lineno)
...
def p_let_body(p):
    ...
    #else:
    #    if(len(p) == 8):
    #        p[0] = [AttributeFeatureNode(p[1], p[3], p[5], p.lineno)] + p[7]
    #    else:
    #        if(p[4] == "<-"):
    #            p[0] = [AttributeFeatureNode(p[1], p[3], p[5], p.lineno)]
    #        else:
    #            p[0] = [AttributeFeatureNode(p[1], p[3], None, p.lineno)] + p[5]
...
def p_case_body(p):
    ...
    #if(len(p) == 7):
    #    p[0] = [CaseBranchNode(p[1], p[3], p[5], p.lineno)]
    #else:
    #    p[0] = [CaseBranchNode(p[1], p[3], p[5], p.lineno)] + p[7]
...
def p_mixed_expression(p):
    ...
    #if(len(p) > 2):
    #    if(p[3] == "Less"):
    #        p[0] = LessNode(p[1], p[3], p.lineno)
    #    else:
    #        if(p[3] == "Equal"):
    #            p[0] = EqualNode(p[1], p[3], p.lineno)
    #        else:
    #            p[0] = LessEqualNode(p[1], p[3], p.lineno)
    #else:
    #    p[0] = p[1]
...
def p_arithmetic_expression(p):
    ...
    #else:
    #    if(p[2] == "Plus"):
    #        p[0] = PlusNode(p[1], p[3], p.lineno)
    #    else:
    #        p[0] = MinusNode(p[1], p[3], p.lineno)
...
def p_term(p):
    ...
    #else:
    #    if(p[2] == "Times"):
    #        p[0] = TimesNode(p[1], p[3], p.lineno)
    #    else:
    #        p[0] = DivideNode(p[1], p[3], p.lineno)
...
def p_factor_extra(p):
    ...
    #    p[0] = p[1]
    #else:
    #    p[0] = ComplementNode(p[2], p.lineno)


def p_program_atom_boolean(p):
    '''program_atom : TRUE
                    | FALSE'''
    #p[0] = ConstantBoolNode(p[1], p.lineno)


def p_program_atom_string(p):
    '''program_atom : STRING'''
    #p[0] = ConstantStringNode(p[1], p.lineno)


def p_program_atom_int(p):
    '''program_atom : NUMBER'''
    #p[0] = ConstantNumericNode(p[1], p.lineno)


def p_program_atom_id(p):
    '''program_atom : ATTRIBUTEID'''
    #p[0] = VariableNode(p[1], p.lineno)
...
def p_program_atom_new(p):
    ...
    #p[0] = NewStatementNode(p[2], p.lineno)
