...
_lr_productions = [
  ...
  ('def_attr -> ID COLON TYPE ASSIGN expr','def_attr',5,'p_def_attr_declaration','parser.py',59),
  ('def_attr -> ID COLON TYPE','def_attr',3,'p_def_attr_declaration','parser.py',60),
  ('def_func -> ID OPAREN params CPAREN COLON TYPE OBRACKET expr CBRACKET','def_func',9,'p_def_func','parser.py',68),
  ('params -> param_list','params',1,'p_params_ne','parser.py',73),
  ('params -> empty','params',1,'p_params_e','parser.py',78),
  ('param_list -> param COMMA param_list','param_list',3,'p_param_list','parser.py',83),
  ('param_list -> param empty','param_list',2,'p_param_list','parser.py',84),
  ('param -> ID COLON TYPE','param',3,'p_param','parser.py',92),
  ('expr -> LET let_attrs IN expr','expr',4,'p_expr_flow','parser.py',97),
  ('expr -> CASE expr OF case_list ESAC','expr',5,'p_expr_flow','parser.py',98),
  ('expr -> IF expr THEN expr ELSE expr FI','expr',7,'p_expr_flow','parser.py',99),
  ('expr -> WHILE expr LOOP expr POOL','expr',5,'p_expr_flow','parser.py',100),
  ('expr -> ID ASSIGN expr','expr',3,'p_expr_assign','parser.py',113),
  ('expr -> expr AT TYPE DOT ID OPAREN arg_list CPAREN','expr',8,'p_expr_func_all','parser.py',118),
  ('expr -> expr DOT ID OPAREN arg_list CPAREN','expr',6,'p_expr_func_all','parser.py',119),
  ('expr -> ID OPAREN arg_list CPAREN','expr',4,'p_expr_func_all','parser.py',120),
  ('expr -> expr PLUS expr','expr',3,'p_expr_operators_binary','parser.py',130),
  ('expr -> expr MINUS expr','expr',3,'p_expr_operators_binary','parser.py',131),
  ('expr -> expr STAR expr','expr',3,'p_expr_operators_binary','parser.py',132),
  ('expr -> expr DIV expr','expr',3,'p_expr_operators_binary','parser.py',133),
  ('expr -> expr LOWER expr','expr',3,'p_expr_operators_binary','parser.py',134),
  ('expr -> expr LOWEREQ expr','expr',3,'p_expr_operators_binary','parser.py',135),
  ('expr -> expr EQUAL expr','expr',3,'p_expr_operators_binary','parser.py',136),
  ('expr -> NOT expr','expr',2,'p_expr_operators_unary','parser.py',154),
  ('expr -> ISVOID expr','expr',2,'p_expr_operators_unary','parser.py',155),
  ('expr -> LNOT expr','expr',2,'p_expr_operators_unary','parser.py',156),
  ('expr -> OPAREN expr CPAREN','expr',3,'p_expr_group','parser.py',166),
  ('expr -> atom','expr',1,'p_expr_atom','parser.py',171),
  ('let_attrs -> def_attr COMMA let_attrs','let_attrs',3,'p_let_attrs','parser.py',176),
  ('let_attrs -> def_attr','let_attrs',1,'p_let_attrs','parser.py',177),
  ('case_list -> case_elem SEMICOLON case_list','case_list',3,'p_case_list','parser.py',185),
  ('case_list -> case_elem SEMICOLON','case_list',2,'p_case_list','parser.py',186),
  ('case_elem -> ID COLON TYPE ARROW expr','case_elem',5,'p_case_elem','parser.py',194),
  ('arg_list -> arg_list_ne','arg_list',1,'p_arg_list','parser.py',199),
  ('arg_list -> empty','arg_list',1,'p_arg_list','parser.py',200),
  ('arg_list_ne -> expr COMMA arg_list_ne','arg_list_ne',3,'p_arg_list_ne','parser.py',205),
  ('arg_list_ne -> expr','arg_list_ne',1,'p_arg_list_ne','parser.py',206),
  ('atom -> INT','atom',1,'p_atom_int','parser.py',214),
  ('atom -> ID','atom',1,'p_atom_id','parser.py',219),
  ('atom -> NEW TYPE','atom',2,'p_atom_new','parser.py',224),
  ('atom -> block','atom',1,'p_atom_block','parser.py',229),
  ('atom -> BOOL','atom',1,'p_atom_bool','parser.py',234),
  ('atom -> STRING','atom',1,'p_atom_atring','parser.py',239),
  ('block -> OBRACKET block_list CBRACKET','block',3,'p_block','parser.py',244),
  ('block_list -> expr SEMICOLON block_list','block_list',3,'p_block_list','parser.py',249),
  ('block_list -> expr SEMICOLON','block_list',2,'p_block_list','parser.py',250),
]
