...
_lr_productions = [
  ...
  ('class_list -> def_class SEMICOLON class_list','class_list',3,'p_class_list','parser.py',30),
  ('class_list -> def_class SEMICOLON','class_list',2,'p_class_list','parser.py',31),
  ('def_class -> CLASS TYPE OBRACKET feature_list CBRACKET','def_class',5,'p_def_class','parser.py',40),
  ('def_class -> CLASS TYPE INHERITS TYPE OBRACKET feature_list CBRACKET','def_class',7,'p_def_class','parser.py',41),
  ...
  ('feature_list -> empty','feature_list',1,'p_feature_list','parser.py',53),
  ('def_attr -> ID COLON TYPE ASSIGN expr','def_attr',5,'p_def_attr_declaration','parser.py',61),
  ('def_attr -> ID COLON TYPE','def_attr',3,'p_def_attr_declaration','parser.py',62),
  ('def_func -> ID OPAREN params CPAREN COLON TYPE OBRACKET expr CBRACKET','def_func',9,'p_def_func','parser.py',72),
  ('params -> param_list','params',1,'p_params_ne','parser.py',78),
  ('params -> empty','params',1,'p_params_e','parser.py',83),
  ('param_list -> param COMMA param_list','param_list',3,'p_param_list','parser.py',88),
  ('param_list -> param empty','param_list',2,'p_param_list','parser.py',89),
  ('param -> ID COLON TYPE','param',3,'p_param','parser.py',97),
  ('expr -> LET let_attrs IN expr','expr',4,'p_expr_flow','parser.py',103),
  ('expr -> CASE expr OF case_list ESAC','expr',5,'p_expr_flow','parser.py',104),
  ('expr -> IF expr THEN expr ELSE expr FI','expr',7,'p_expr_flow','parser.py',105),
  ('expr -> WHILE expr LOOP expr POOL','expr',5,'p_expr_flow','parser.py',106),
  ('expr -> ID ASSIGN expr','expr',3,'p_expr_assign','parser.py',121),
  ('expr -> expr AT TYPE DOT ID OPAREN arg_list CPAREN','expr',8,'p_expr_func_all','parser.py',127),
  ('expr -> expr DOT ID OPAREN arg_list CPAREN','expr',6,'p_expr_func_all','parser.py',128),
  ('expr -> ID OPAREN arg_list CPAREN','expr',4,'p_expr_func_all','parser.py',129),
  ('expr -> expr PLUS expr','expr',3,'p_expr_operators_binary','parser.py',150),
  ('expr -> expr MINUS expr','expr',3,'p_expr_operators_binary','parser.py',151),
  ('expr -> expr STAR expr','expr',3,'p_expr_operators_binary','parser.py',152),
  ('expr -> expr DIV expr','expr',3,'p_expr_operators_binary','parser.py',153),
  ('expr -> expr LOWER expr','expr',3,'p_expr_operators_binary','parser.py',154),
  ('expr -> expr LOWEREQ expr','expr',3,'p_expr_operators_binary','parser.py',155),
  ('expr -> expr EQUAL expr','expr',3,'p_expr_operators_binary','parser.py',156),
  ('expr -> NOT expr','expr',2,'p_expr_operators_unary','parser.py',176),
  ('expr -> ISVOID expr','expr',2,'p_expr_operators_unary','parser.py',177),
  ('expr -> LNOT expr','expr',2,'p_expr_operators_unary','parser.py',178),
  ('expr -> OPAREN expr CPAREN','expr',3,'p_expr_group','parser.py',190),
  ('expr -> atom','expr',1,'p_expr_atom','parser.py',195),
  ('let_attrs -> def_attr COMMA let_attrs','let_attrs',3,'p_let_attrs','parser.py',200),
  ('let_attrs -> def_attr','let_attrs',1,'p_let_attrs','parser.py',201),
  ('case_list -> case_elem SEMICOLON case_list','case_list',3,'p_case_list','parser.py',209),
  ('case_list -> case_elem SEMICOLON','case_list',2,'p_case_list','parser.py',210),
  ('case_elem -> ID COLON TYPE ARROW expr','case_elem',5,'p_case_elem','parser.py',218),
  ('arg_list -> arg_list_ne','arg_list',1,'p_arg_list','parser.py',224),
  ('arg_list -> empty','arg_list',1,'p_arg_list','parser.py',225),
  ('arg_list_ne -> expr COMMA arg_list_ne','arg_list_ne',3,'p_arg_list_ne','parser.py',230),
  ('arg_list_ne -> expr','arg_list_ne',1,'p_arg_list_ne','parser.py',231),
  ('atom -> INT','atom',1,'p_atom_int','parser.py',239),
  ('atom -> ID','atom',1,'p_atom_id','parser.py',245),
  ('atom -> NEW TYPE','atom',2,'p_atom_new','parser.py',251),
  ('atom -> block','atom',1,'p_atom_block','parser.py',257),
  ('atom -> BOOL','atom',1,'p_atom_bool','parser.py',262),
  ('atom -> STRING','atom',1,'p_atom_atring','parser.py',268),
  ('block -> OBRACKET block_list CBRACKET','block',3,'p_block','parser.py',274),
  ('block_list -> expr SEMICOLON block_list','block_list',3,'p_block_list','parser.py',279),
  ('block_list -> expr SEMICOLON','block_list',2,'p_block_list','parser.py',280),
]
