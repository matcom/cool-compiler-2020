...
def update_method_param(current_type : Type, method: str, param_name: str, new_type: Type):
    m = current_type.methods[method]
    for i,(pname, ptype) in enumerate(zip(m.param_names, m.param_types)):
        if pname == param_name:
            m.param_types[i] = new_type

def update_scope_variable(vname: str, new_type: Type, scope: Scope, index= None):
    if not index:
        index = 0
