...
BAD_ERROR_FORMAT = "El error no esta en formato: (<línea>,<columna>) - "
BAD_ERROR_FORMAT += "<tipo_de_error>: <texto_del_error> o no se encuentra "
BAD_ERROR_FORMAT += "en la 3ra linea\n\n%s"
UNEXPECTED_ERROR = "Se esperaba un %s en (%d, %d). "
UNEXPECTED_ERROR += "Su error fue un %s en (%d, %d)"
...
def get_file_name(path: str):
    try:
        return path[path.rindex("/") + 1 :]  # noqa
    except ValueError:
        return path
...
def compare_errors(
    ...
    if error_file_path:
        ...
        assert return_code == 0, TEST_MUST_COMPILE % get_file_name(
            cool_file_path,
        )
