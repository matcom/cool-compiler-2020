...
function_build_dispatch_tables:
	...
	sw $t1 44($v0)
	la $t1 function_Complex_equal
	sw $t1 48($v0)
	la $t1 function_Complex_x_value
	sw $t1 52($v0)
	la $t1 function_Complex_y_value
	sw $t1 56($v0)
	lw $t0 32($s0)
	sw $v0 8($t0)
