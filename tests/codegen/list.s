...
function_build_dispatch_tables:
	...
	la $t1 function_IO_out_string
	sw $t1 24($v0)
	la $t1 function_Main_print_list
	sw $t1 28($v0)
	...
	sw $t1 12($v0)
	la $t1 function_List_head
	sw $t1 16($v0)
	la $t1 function_List_tail
	sw $t1 20($v0)
	...
	sw $t1 28($v0)
	la $t1 function_Cons_head
	sw $t1 32($v0)
	la $t1 function_Cons_tail
	sw $t1 36($v0)
